def find_language_information(languages, language_name)
  ...
  # }
  selection = languages.select { |lang| lang == language_name }
  selection[language_name]
end
...
def find_beautiful_languages(languages)
  ...
  selection = languages.select { |language, info| info[:is_beautiful?] }
  selection
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  # }
  result = nil
  selected_language = languages.select { |language| language == language_name }
  selected_language.each do |language, info|
    result = info.dig(:facts, fact_index)
  end
  result
end
